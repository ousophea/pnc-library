...
<div class="row">
	<div class="box box-solid box-default panel-sm ui-shadow">
		...
		<div class="box-body">
			<?php foreach ($gbarcode as $value) {
					$this->session->set_userdata('barcode', $value->b_barcode);
			}?>
			<form action="<?php echo base_url('BorrowBook/index');?>" method = "post">
				<label for="">Barcode</label>
				<input type="text" name="com_barcode" class="form-control" placeholder="1234567" value="<?php echo $this->session->userdata('barcode');?>"/>
			</form>
			<p class="clearfix"></p>
			<form action="<?php echo base_url('BorrowBook/insert_borrow'); ?>" method="post">
				<?php foreach ($rule as $key) {$this->session->set_userdata('num_day', $key->rul_num_day);?>
